...
class V31StateMachine(
    ...
    private data class EndConfirmSample(
        val tsMs: Long,
        val distanceM: Float,
    ...
    private data class EndConfirmQuality(
        val accepted: Boolean,
        val sampleCount: Int,
        val medianDistanceM: Float?,
        val rangeM: Float?,
        val sigmaM: Float?,
        val worldSpreadM: Float?,
        val reticleDriftPx: Float?,
        val latestAgeMs: Long?,
        val reason: String
    )

    private data class ShortLiveConfirmQuality(
        val accepted: Boolean,
        val reason: String,
        val sampleCount: Int,
        val representativeDistanceM: Float?,
        val rangeM: Float?,
        val sigmaM: Float?,
        val reticleDriftPx: Float?,
        val latestAgeMs: Long?
    ...
    data class UiModel(
        ...
        val distanceFinalizePolicy: String? = null,
        val distanceFinalizeReason: String? = null,
        val distanceFinalizeAccepted: Boolean? = null,
        val distanceFinalizeSampleCount: Int? = null,
        val distanceFinalizeWindowMs: Long? = null,
        val distanceFinalizeRangeM: Float? = null,
        val distanceFinalizeSigmaM: Float? = null,
        val distanceFinalizeWorldSpreadM: Float? = null,
        val distanceFinalizeReticleDriftPx: Float? = null,
        val distanceFinalizeLatestAgeMs: Long? = null,
        val distanceFinalizePrimaryPolicy: String? = null,
        val distanceFinalizeSource: String? = null,
        val distanceFinalizeShortAccepted: Boolean? = null,
        val distanceFinalizeShortReason: String? = null,
        val distanceFinalizeShortSampleCount: Int? = null,
        val distanceFinalizeShortRangeM: Float? = null,
        val distanceFinalizeShortSigmaM: Float? = null,
        val distanceFinalizeShortReticleDriftPx: Float? = null,
        val distanceFinalizeShortLatestAgeMs: Long? = null,
    ...
    // AR warm-up: 5-point plane evidence (admission only; distance plane set at BALL fix)
    private val WARMUP_PROBE_OFFSET_RATIO = 0.12f
    ...
    private val BALL_WARMUP_OUTDOOR_REQUIRED_HITS = 24
    private val BALL_WARMUP_OUTDOOR_REQUIRED_HITS_INITIAL = 18
    ...
    private var lastWarmupGateLogNs: Long = 0L
    ...
    private var liveJumpRejectedLastFrame: Boolean = false
    private val endConfirmBuffer = ArrayDeque<EndConfirmSample>(48)
    private var lastEndConfirmQuality: EndConfirmQuality? = null
    private var lastShortConfirmQuality: ShortLiveConfirmQuality? = null
    private var lastDistanceFinalizeSource: String? = null
    private var endConfirmLastHitSource: String? = null
    private var endConfirmNoHitStreak: Int = 0
    ...
    private fun pushEndConfirmSample(
        nowNs: Long,
        distanceM: Float,
        trackingOk: Boolean,
        ...
        endConfirmBuffer.addLast(
            EndConfirmSample(
                tsMs = nowNs / 1_000_000L,
                distanceM = distanceM,
        ...
        val cutoff = (nowNs / 1_000_000L) - 2000L
        while (endConfirmBuffer.isNotEmpty() && endConfirmBuffer.first().tsMs < cutoff) {
            endConfirmBuffer.removeFirst()
        }
    }

    private fun resolveEndConfirmLimits(estimatedDistanceM: Float): Pair<Float, Float> {
        return if (estimatedDistanceM >= 6f) {
            0.25f to 0.12f
        } else {
            0.15f to 0.08f
        }
    ...
    private fun evaluateRecentEndConfirm(nowNs: Long, windowMs: Long = 800L, minSamples: Int = 5): EndConfirmQuality {
        val nowMs = nowNs / 1_000_000L
        val recent = endConfirmBuffer.filter { nowMs - it.tsMs <= windowMs }
        val filtered =
            recent.filter { it.trackingOk && it.distanceValid && !it.jumpRejected && it.distanceM.isFinite() && it.distanceM > 0f }
        if (filtered.size < minSamples) {
            return EndConfirmQuality(
                accepted = false,
        ...
        val latestTs = filtered.maxOfOrNull { it.tsMs } ?: 0L
        val latestAgeMs = (nowMs - latestTs).coerceAtLeast(0L)
        if (latestAgeMs > 220L) {
            return EndConfirmQuality(
                accepted = false,
                sampleCount = filtered.size,
        ...
        val sourceCounts = filtered.groupingBy { it.hitSource }.eachCount()
        val dominant = sourceCounts.maxByOrNull { it.value }?.key
        val dominantCount = dominant?.let { sourceCounts[it] } ?: 0
        val dominantRatio = if (filtered.isNotEmpty()) dominantCount.toFloat() / filtered.size.toFloat() else 0f
        if (dominant == null || dominantRatio < 0.7f) {
            return EndConfirmQuality(
                accepted = false,
                sampleCount = filtered.size,
                medianDistanceM = null,
                ...
                reticleDriftPx = null,
                latestAgeMs = latestAgeMs,
                reason = "mixed_hit_source"
            )
        }
        val continuity = filtered.filter { it.hitSource == dominant }
        if (continuity.size < minSamples) {
            return EndConfirmQuality(
                accepted = false,
                sampleCount = continuity.size,
                medianDistanceM = null,
                rangeM = null,
                sigmaM = null,
                worldSpreadM = null,
                reticleDriftPx = null,
                latestAgeMs = latestAgeMs,
                reason = "insufficient_continuity_samples"
            )
        }
        if (continuity.any {
                !it.worldX.isFinite() || !it.worldY.isFinite() || !it.worldZ.isFinite() ||
                    !it.reticleX.isFinite() || !it.reticleY.isFinite()
            }) {
            return EndConfirmQuality(
                accepted = false,
                sampleCount = continuity.size,
                ...
                reticleDriftPx = null,
                latestAgeMs = latestAgeMs,
                reason = "invalid_world_or_reticle"
            )
        }
        val sorted = continuity.map { it.distanceM }.sorted()
        val median = if (sorted.size % 2 == 1) sorted[sorted.size / 2] else (sorted[(sorted.size / 2) - 1] + sorted[sorted.size / 2]) * 0.5f
        val max = sorted.last()
        val min = sorted.first()
        val range = max - min
        val mean = sorted.average().toFloat()
        val variance = sorted.map { (it - mean) * (it - mean) }.average().toFloat()
        val sigma = sqrt(variance)
        val spreadX = continuity.maxOf { it.worldX } - continuity.minOf { it.worldX }
        val spreadY = continuity.maxOf { it.worldY } - continuity.minOf { it.worldY }
        val spreadZ = continuity.maxOf { it.worldZ } - continuity.minOf { it.worldZ }
        val worldSpread = sqrt(spreadX * spreadX + spreadY * spreadY + spreadZ * spreadZ)
        val driftX = continuity.maxOf { it.reticleX } - continuity.minOf { it.reticleX }
        val driftY = continuity.maxOf { it.reticleY } - continuity.minOf { it.reticleY }
        val reticleDrift = sqrt(driftX * driftX + driftY * driftY)
        val (rangeLimit, sigmaLimit) = resolveEndConfirmLimits(median)
        val worldSpreadLimit = if (median < 6f) 0.08f else 0.15f
        val accepted = range <= rangeLimit && sigma <= sigmaLimit && worldSpread <= worldSpreadLimit && reticleDrift <= 30f
        val reason =
            when {
                range > rangeLimit -> "range_too_large"
                sigma > sigmaLimit -> "sigma_too_large"
                worldSpread > worldSpreadLimit -> "point_spread_too_large"
                reticleDrift > 30f -> "reticle_drift_too_large"
                else -> "ok"
            }
        return EndConfirmQuality(
            accepted = accepted,
            ...
            rangeM = range,
            sigmaM = sigma,
            worldSpreadM = worldSpread,
            reticleDriftPx = reticleDrift,
            latestAgeMs = latestAgeMs,
            reason = reason
        )
    }

    private fun evaluateShortLiveConfirm(nowNs: Long, windowMs: Long = 300L, minSamples: Int = 3): ShortLiveConfirmQuality {
        val nowMs = nowNs / 1_000_000L
        val recent = endConfirmBuffer.filter { nowMs - it.tsMs <= windowMs }
        val filtered =
            recent.filter { it.trackingOk && it.distanceValid && !it.jumpRejected && it.distanceM.isFinite() && it.distanceM > 0f }
        if (filtered.size < minSamples) {
            return ShortLiveConfirmQuality(false, "insufficient_samples", filtered.size, null, null, null, null, null)
        }
        val latestTs = filtered.maxOfOrNull { it.tsMs } ?: 0L
        val latestAgeMs = (nowMs - latestTs).coerceAtLeast(0L)
        if (latestAgeMs > 220L) {
            return ShortLiveConfirmQuality(false, "stale_sample", filtered.size, null, null, null, null, latestAgeMs)
        }
        val sourceCounts = filtered.groupingBy { it.hitSource }.eachCount()
        val dominant = sourceCounts.maxByOrNull { it.value }?.key
        val dominantCount = dominant?.let { sourceCounts[it] } ?: 0
        val dominantRatio = if (filtered.isNotEmpty()) dominantCount.toFloat() / filtered.size.toFloat() else 0f
        if (dominant == null || dominantRatio < 0.7f) {
            return ShortLiveConfirmQuality(false, "mixed_hit_source", filtered.size, null, null, null, null, latestAgeMs)
        }
        val continuity = filtered.filter { it.hitSource == dominant }
        if (continuity.size < minSamples) {
            return ShortLiveConfirmQuality(false, "insufficient_continuity_samples", continuity.size, null, null, null, null, latestAgeMs)
        }
        if (continuity.any { !it.reticleX.isFinite() || !it.reticleY.isFinite() }) {
            return ShortLiveConfirmQuality(false, "invalid_reticle", continuity.size, null, null, null, null, latestAgeMs)
        }
        val sorted = continuity.map { it.distanceM }.sorted()
        val representative = if (sorted.size % 2 == 1) sorted[sorted.size / 2] else (sorted[(sorted.size / 2) - 1] + sorted[sorted.size / 2]) * 0.5f
        val range = (sorted.lastOrNull() ?: representative) - (sorted.firstOrNull() ?: representative)
        val mean = sorted.average().toFloat()
        val variance = sorted.map { (it - mean) * (it - mean) }.average().toFloat()
        val sigma = sqrt(variance)
        val driftX = continuity.maxOf { it.reticleX } - continuity.minOf { it.reticleX }
        val driftY = continuity.maxOf { it.reticleY } - continuity.minOf { it.reticleY }
        val reticleDrift = sqrt(driftX * driftX + driftY * driftY)
        val (rangeLimit, sigmaLimit) = resolveEndConfirmLimits(representative)
        val accepted = range <= rangeLimit && sigma <= sigmaLimit && reticleDrift <= 30f
        val reason =
            when {
                range > rangeLimit -> "range_too_large"
                sigma > sigmaLimit -> "sigma_too_large"
        ...
        return ShortLiveConfirmQuality(
            accepted = accepted,
            reason = reason,
            sampleCount = continuity.size,
            representativeDistanceM = representative,
            rangeM = range,
            sigmaM = sigma,
            reticleDriftPx = reticleDrift,
            latestAgeMs = latestAgeMs
    ...
    private fun currentBallFixNeedHits(): Int {
        if (ProductFlags.ZOOM_DISABLED) return BALL_FIX_NEED_HITS
    ...
    private data class WarmupProbeResult(val success: Boolean, val hitCount: Int)

    private fun warmupProbePoints(roi: RectF): List<Pair<Float, Float>> {
        val cx = roi.centerX()
        val cy = roi.centerY()
        val dx = roi.width() * WARMUP_PROBE_OFFSET_RATIO
        val dy = roi.height() * WARMUP_PROBE_OFFSET_RATIO
        return listOf(
            cx to cy,
            (cx - dx) to cy,
            (cx + dx) to cy,
    ...
    private fun evaluateWarmupPlaneEvidence(frame: Frame, roi: RectF): WarmupProbeResult {
        var hitCount = 0
        for ((x, y) in warmupProbePoints(roi)) {
            val hit =
        ...
        return WarmupProbeResult(success = hitCount > 0, hitCount = hitCount)
    }

    private fun warmupRequiredHits(nowNs: Long = System.nanoTime()): Int {
        val initialRelax =
            warmupSessionStartNs > 0L &&
                (nowNs - warmupSessionStartNs) / 1_000_000L < BALL_WARMUP_INITIAL_RELAX_MS
        return if (ProductFlags.OUTDOOR_WARMUP_RELAXED) {
            if (initialRelax) BALL_WARMUP_OUTDOOR_REQUIRED_HITS_INITIAL else BALL_WARMUP_OUTDOOR_REQUIRED_HITS
        } else {
            if (initialRelax) BALL_WARMUP_REQUIRED_HITS_INITIAL else BALL_WARMUP_REQUIRED_HITS
        }
    }

    private fun logWarmupGate(
        frame: Frame,
        ...
        if (nowNs - lastWarmupGateLogNs < throttleNs) return
        lastWarmupGateLogNs = nowNs
        val successCount = arWarmupWindow.count { it }
        val required = warmupRequiredHits(nowNs)
        Log.i(
            "WARMUP_MULTI",
            "hitCount=$hitCount success=$planeHit required=$required ready=$arWarmupReady " +
                "windowSuccessCount=$successCount"
        )
        val reason = frame.camera.trackingFailureReason
        Log.i(
            "WARMUP",
            "tracking=${tracking.name} reason=${reason.name} outdoorRelaxed=${ProductFlags.OUTDOOR_WARMUP_RELAXED} " +
                "planeHit=$planeHit hitCount=$hitCount successCount=$successCount/$required " +
                "window=$BALL_WARMUP_WINDOW_FRAMES arWarmupReady=$arWarmupReady"
        )
    }
    ...
    fun tick(frame: Frame, roiScreen: RectF, nowNs: Long, session: Session): UiModel {
        ...
        if (!sampling) {
            // AR warm-up: IDLE 5-point plane evidence (admission only)
            if (state == State.IDLE && tracking == TrackingState.TRACKING) {
                if (warmupSessionStartNs == 0L) warmupSessionStartNs = System.nanoTime()
                val warmupProbe = evaluateWarmupPlaneEvidence(frame, roiScreen)
                arWarmupWindow.addLast(warmupProbe.success)
                while (arWarmupWindow.size > BALL_WARMUP_WINDOW_FRAMES) arWarmupWindow.removeFirst()
                val requiredHits = warmupRequiredHits(nowNs)
                val wasWarmupReady = arWarmupReady
                val successCount = arWarmupWindow.count { it }
                arWarmupReady = successCount >= requiredHits
                if (!arWarmupReady) {
                    logWarmupGate(frame, tracking, warmupProbe.success, warmupProbe.hitCount, nowNs)
                } else if (!wasWarmupReady) {
                    Log.i(
                        "WARMUP_MULTI",
                        "hitCount=${warmupProbe.hitCount} success=${warmupProbe.success} " +
                            "required=$requiredHits ready=true windowSuccessCount=$successCount"
                    )
                }
            }
            return buildUi(nowNs, tracking)
        ...
        if (inLiveStates && startAnchor != null) {
            ...
            liveJumpRejectedLastFrame = false
            ...
            if (raw != null && raw!!.isFinite() && raw!! > 0f) {
                val currentSource = liveSource.name
                if (endConfirmLastHitSource != null && endConfirmLastHitSource != currentSource) {
                    endConfirmBuffer.clear()
                }
                endConfirmLastHitSource = currentSource
                endConfirmNoHitStreak = 0
                val cur = raw!!
                liveRawMeters = cur
                centerHitValid = true
                if (!liveHasValue) {
                    ...
                    if (abs(curClamped - prev) <= jumpLimit) {
                        ...
                        liveJumpRejectedLastFrame = true
                        if (abs(curClamped - prev) > 0.30f) {
                            endConfirmBuffer.clear()
                        }
                    }
                }
                ...
                val world = lastLiveCupWorldForDistance
                pushEndConfirmSample(
                    nowNs = nowNs,
                    distanceM = liveSmoothedMeters,
                    trackingOk = tracking == TrackingState.TRACKING,
                    distanceValid = centerHitValid == true,
                    jumpRejected = liveJumpRejectedLastFrame,
                    hitSource = liveSource.name,
                    worldX = world?.getOrNull(0) ?: Float.NaN,
                    worldY = world?.getOrNull(1) ?: Float.NaN,
                    worldZ = world?.getOrNull(2) ?: Float.NaN,
                    reticleX = roiScreen.centerX(),
                    reticleY = roiScreen.centerY(),
                    sigmaM = liveStabilitySigmaAndRangeOrNull()?.first
                )
            ...
            endConfirmNoHitStreak++
            if (tracking != TrackingState.TRACKING || endConfirmNoHitStreak >= 3) {
                endConfirmBuffer.clear()
            }
            liveHasValue = false
            ...
            endConfirmBuffer.clear()
            lastShortConfirmQuality = null
            lastEndConfirmQuality = null
            lastDistanceFinalizeSource = null
        }
        ...
        if (state == State.AIM_START || state == State.AIM_END) {
            ...
            if (state == State.AIM_END && finishRequestPending) {
                if (ProductFlags.DISTANCE_ONLY_PRODUCT) {
                    val shortQ = evaluateShortLiveConfirm(nowNs = nowNs, windowMs = 300L, minSamples = 3)
                    lastShortConfirmQuality = shortQ
                    if (shortQ.accepted && shortQ.representativeDistanceM != null) {
                        endLiveSnapshotMeters = shortQ.representativeDistanceM
                        capturedDistanceLockLiveSource = "SHORT_LIVE_CONFIRM"
                        capturedDistanceLockLiveRawM = liveRawMeters
                        capturedDistanceLockLiveSigmaM = shortQ.sigmaM
                        capturedDistanceLockLiveRangeM = shortQ.rangeM
                        capturedDistanceLockLiveStable = true
                        capturedDistanceFinalFallbackUsed = false
                        capturedDistanceFinalSnapshotReason = "SHORT_LIVE_CONFIRM"
                        lastDistanceFinalizeSource = "SHORT_LIVE_CONFIRM"
                        finishRequestPending = false
                        state = State.END_LOCKED
                        endLockedAtNs = nowNs
                        Log.d(
                            "DistanceMode",
                            "distanceFinalizePrimaryPolicy=SHORT_LIVE_CONFIRM distanceFinalizeSource=SHORT_LIVE_CONFIRM " +
                                "distanceFinalizeShortAccepted=true distanceFinalizeShortReason=${shortQ.reason} " +
                                "distanceFinalizeShortSampleCount=${shortQ.sampleCount} distanceFinalizeShortRangeM=${shortQ.rangeM ?: -1f} " +
                                "distanceFinalizeShortSigmaM=${shortQ.sigmaM ?: -1f} distanceFinalizeShortReticleDriftPx=${shortQ.reticleDriftPx ?: -1f} " +
                                "distanceFinalizeShortLatestAgeMs=${shortQ.latestAgeMs ?: -1L}"
                        )
                        return buildUi(nowNs, tracking, sample, flashLock = true)
                    }
                    val recentQ = evaluateRecentEndConfirm(nowNs = nowNs, windowMs = 800L, minSamples = 5)
                    lastEndConfirmQuality = recentQ
                    if (recentQ.accepted && recentQ.medianDistanceM != null) {
                        endLiveSnapshotMeters = recentQ.medianDistanceM
                        capturedDistanceLockLiveSource = "RECENT_LIVE_MEDIAN_FALLBACK"
                        capturedDistanceLockLiveRawM = liveRawMeters
                        capturedDistanceLockLiveSigmaM = recentQ.sigmaM
                        capturedDistanceLockLiveRangeM = recentQ.rangeM
                        capturedDistanceLockLiveStable = true
                        capturedDistanceFinalFallbackUsed = true
                        capturedDistanceFinalSnapshotReason = "RECENT_LIVE_MEDIAN_FALLBACK"
                        lastDistanceFinalizeSource = "RECENT_LIVE_MEDIAN_FALLBACK"
                        finishRequestPending = false
                        state = State.END_LOCKED
                        endLockedAtNs = nowNs
                        Log.d(
                            "DistanceMode",
                            "distanceFinalizePrimaryPolicy=SHORT_LIVE_CONFIRM distanceFinalizeSource=RECENT_LIVE_MEDIAN_FALLBACK " +
                                "distanceFinalizeShortAccepted=false distanceFinalizeShortReason=${shortQ.reason} " +
                                "distanceFinalizeShortSampleCount=${shortQ.sampleCount} distanceFinalizeReason=${recentQ.reason} distanceFinalizeAccepted=true"
                        )
                        return buildUi(nowNs, tracking, sample, flashLock = true)
                    }
                    finishRequestPending = false
                    Log.d(
                        "DistanceMode",
                        "distanceFinalizePrimaryPolicy=SHORT_LIVE_CONFIRM distanceFinalizeSource=REJECTED " +
                            "distanceFinalizeShortAccepted=false distanceFinalizeShortReason=${shortQ.reason} " +
                            "distanceFinalizeShortSampleCount=${shortQ.sampleCount} distanceFinalizeReason=${recentQ.reason} distanceFinalizeAccepted=false"
                    )
                    return buildUi(nowNs, tracking, sample)
                }
            ...
            if (nowNs - stabilizingEnterNs >= timeoutNs) {
                ...
                if (state == State.STABILIZING_END) {
                    if (ProductFlags.DISTANCE_ONLY_PRODUCT) {
                        ...
                        if (shortQ.accepted && shortQ.representativeDistanceM != null) {
                            endLiveSnapshotMeters = shortQ.representativeDistanceM
                            capturedDistanceLockLiveSource = "SHORT_LIVE_CONFIRM_TIMEOUT"
                            capturedDistanceLockLiveRawM = liveRawMeters
                            capturedDistanceLockLiveSigmaM = shortQ.sigmaM
                            capturedDistanceLockLiveRangeM = shortQ.rangeM
                            capturedDistanceLockLiveStable = true
                            capturedDistanceFinalFallbackUsed = true
                            capturedDistanceFinalSnapshotReason = "SHORT_LIVE_CONFIRM_TIMEOUT"
                            lastDistanceFinalizeSource = "SHORT_LIVE_CONFIRM_TIMEOUT"
                            state = State.END_LOCKED
                            endLockedAtNs = nowNs
                            return buildUi(nowNs, tracking, sample, flashLock = true)
                        }
                        val q = evaluateRecentEndConfirm(nowNs = nowNs, windowMs = 800L, minSamples = 5)
                        lastEndConfirmQuality = q
                        if (q.accepted && q.medianDistanceM != null) {
                            endLiveSnapshotMeters = q.medianDistanceM
                            capturedDistanceLockLiveSource = "RECENT_LIVE_MEDIAN_TIMEOUT_FALLBACK"
                            capturedDistanceLockLiveRawM = liveRawMeters
                            capturedDistanceLockLiveSigmaM = q.sigmaM
                            capturedDistanceLockLiveRangeM = q.rangeM
                            capturedDistanceLockLiveStable = true
                            capturedDistanceFinalFallbackUsed = true
                            capturedDistanceFinalSnapshotReason = "RECENT_LIVE_MEDIAN_TIMEOUT_FALLBACK"
                            lastDistanceFinalizeSource = "RECENT_LIVE_MEDIAN_TIMEOUT_FALLBACK"
                            state = State.END_LOCKED
                            endLockedAtNs = nowNs
                            return buildUi(nowNs, tracking, sample, flashLock = true)
                        }
            ...
            if (buf.size >= fixedMinSamples) {
                ...
                if (sigmaOkConsecutive >= LOCK_CONSEC_TICKS && okElapsed >= LOCK_TIME_GATE_NS) {
                    ...
                    if (state == State.STABILIZING_END) {
                        ...
                        if (qualityBlocked && !ProductFlags.DISTANCE_ONLY_PRODUCT) {
                        ...
                        val liveReady =
                            liveSource == LiveSource.PLANE_INTERSECTION &&
                                ...
                                liveEma.isFinite() &&
                                liveEma > 0f
                        if (!ProductFlags.DISTANCE_ONLY_PRODUCT && liveReady) {
                        ...
                        if (farMode) {
                            ...
                            if (!liveMedianValid || !cupFixValid) {
                                ...
                                if (decision == "HOLD" && !ProductFlags.DISTANCE_ONLY_PRODUCT) {
                                    if (debugLoggingEnabled && nowNs - lastCupFixAttemptLogNs >= 500_000_000L) {
                                        lastCupFixAttemptLogNs = nowNs
                                        Log.d("CUP_FIX_ATTEMPT", "state=$state validHits=${sample.validHits} centerFallbackUsed=${sample.centerFallbackUsed == true} " +
                                            "sigma=${lastSigmaUsedMeters?.let { "%.3f".format(it) } ?: "null"} sigmaMax=${lastSigmaMaxMeters?.let { "%.3f".format(it) } ?: "null"} " +
                    ...
                    var cupWorldForCommit = cupLiveWorldEligibleForEndCommit(nowNs)
                    if (cupWorldForCommit == null && ProductFlags.DISTANCE_ONLY_PRODUCT) {
                        cupWorldForCommit =
                            lastLiveCupWorldForDistance?.copyOf()
                                ?: world3FromPose(stabilizingHit.hitPose)
                    }
    ...
    private fun confirmLock(
        ...
        when (state) {
            State.STABILIZING_START -> {
                ...
                val frame = tickFrame
                if (!ProductFlags.DISTANCE_ONLY_PRODUCT && roi != null && frame != null && axisMode == AxisMode.XYZ) {
            ...
            State.STABILIZING_END -> {
                ...
                if (!ProductFlags.DISTANCE_ONLY_PRODUCT && cupRoi != null && cupFrame != null && axisMode == AxisMode.XYZ) {
    ...
    private fun cupLiveWorldEligibleForEndCommit(nowNs: Long): FloatArray? {
        ...
        if (lastLiveCupWorldUpdateNs <= 0L) {
            return if (ProductFlags.DISTANCE_ONLY_PRODUCT) w.copyOf() else null
        }
        val stale = nowNs - lastLiveCupWorldUpdateNs > CUP_LIVE_WORLD_MAX_STALE_NS
        if (stale && !ProductFlags.DISTANCE_ONLY_PRODUCT) return null
        return w.copyOf()
    }
    ...
    private fun refreshBallGateSnapshot(nowNs: Long, tracking: TrackingState, sample: V31HitSampler.Sample?) {
        ...
        val requiredWarmup = warmupRequiredHits()
    ...
    private fun buildUi(
        ...
        val slopeProjected: SlopeInputProjection.Result? =
            if (!ProductFlags.DISTANCE_ONLY_PRODUCT &&
                (state == State.END_LOCKED || state == State.RESULT) &&
        ...
        val finishEnabled =
            when (state) {
                ...
                State.FAIL ->
                    if (ProductFlags.DISTANCE_ONLY_PRODUCT) {
                        trackingOk && canRetryCupFromFail() && startAnchor != null
                    } else {
                        trackingOk &&
                            canRetryCupFromFail() &&
                            isCupAimReady(sample)
        ...
        val uiModel = UiModel(
            ...
            ballArWarmupRequired =
                if (!startEnabled && (state == State.IDLE || state == State.AIM_START)) {
                    warmupRequiredHits(nowNs)
                } else {
                    null
                },
            ...
            distanceFinalizePolicy =
                if (state == State.RESULT && ProductFlags.DISTANCE_ONLY_PRODUCT) "RECENT_LIVE_MEDIAN" else null,
            distanceFinalizeReason = if (state == State.RESULT) lastEndConfirmQuality?.reason else null,
            distanceFinalizeAccepted = if (state == State.RESULT) lastEndConfirmQuality?.accepted else null,
            distanceFinalizeSampleCount = if (state == State.RESULT) lastEndConfirmQuality?.sampleCount else null,
            distanceFinalizeWindowMs = if (state == State.RESULT && ProductFlags.DISTANCE_ONLY_PRODUCT) 800L else null,
            distanceFinalizeRangeM = if (state == State.RESULT) lastEndConfirmQuality?.rangeM else null,
            distanceFinalizeSigmaM = if (state == State.RESULT) lastEndConfirmQuality?.sigmaM else null,
            distanceFinalizeWorldSpreadM = if (state == State.RESULT) lastEndConfirmQuality?.worldSpreadM else null,
            distanceFinalizeReticleDriftPx = if (state == State.RESULT) lastEndConfirmQuality?.reticleDriftPx else null,
            distanceFinalizeLatestAgeMs = if (state == State.RESULT) lastEndConfirmQuality?.latestAgeMs else null,
            distanceFinalizePrimaryPolicy =
                if (state == State.RESULT && ProductFlags.DISTANCE_ONLY_PRODUCT) "SHORT_LIVE_CONFIRM" else null,
            distanceFinalizeSource = if (state == State.RESULT) lastDistanceFinalizeSource else null,
            distanceFinalizeShortAccepted = if (state == State.RESULT) lastShortConfirmQuality?.accepted else null,
            distanceFinalizeShortReason = if (state == State.RESULT) lastShortConfirmQuality?.reason else null,
            distanceFinalizeShortSampleCount = if (state == State.RESULT) lastShortConfirmQuality?.sampleCount else null,
            distanceFinalizeShortRangeM = if (state == State.RESULT) lastShortConfirmQuality?.rangeM else null,
            distanceFinalizeShortSigmaM = if (state == State.RESULT) lastShortConfirmQuality?.sigmaM else null,
            distanceFinalizeShortReticleDriftPx = if (state == State.RESULT) lastShortConfirmQuality?.reticleDriftPx else null,
            distanceFinalizeShortLatestAgeMs = if (state == State.RESULT) lastShortConfirmQuality?.latestAgeMs else null,
        ...
        if (!ProductFlags.DISTANCE_ONLY_PRODUCT &&
            state == State.RESULT &&
            lastEngineStateForSlopeFieldLog != State.RESULT) {
        ...
        if (ProductFlags.DISTANCE_ONLY_PRODUCT) {
            ...
            val livePlaneCandidate =
                when {
                    ...
                    anchorM.isFinite() && anchorM > 0f -> anchorM
                    else -> 0f
                }
            val resolved =
                when {
                    livePlaneCandidate.isFinite() && livePlaneCandidate > 0f -> livePlaneCandidate
                    anchorM.isFinite() && anchorM > 0f -> anchorM
                    lastDisplayDistanceMeters.isFinite() && lastDisplayDistanceMeters > 0f ->
                        lastDisplayDistanceMeters
                    else -> 0f
                }
            finalDistanceMeters = resolved
            capturedFinalDistanceLivePlaneMeters =
                endLiveSnapshotMeters.takeIf { it.isFinite() && it > 0f } ?: resolved
            capturedFinalDistanceSourceBeforeGuard = "DISTANCE_ONLY_LIVE"
            capturedFinalDistanceSourceAfterGuard = "DISTANCE_ONLY_LIVE"
            capturedFinalDistanceGuardTriggered = false
            capturedFinalDistanceGuardReasons = null
            capturedFinalDistanceAnchorInvalidReason = null
            capturedPlaneVsAnchorDeltaM = null
            capturedPlaneVsAnchorDeltaRatio = null
            Log.d("DistanceMode", "DISTANCE_ONLY final=live_stable")
            return
        }
        val anchorM = distanceBetweenAnchorsMeters()
        val livePlaneCandidate =
