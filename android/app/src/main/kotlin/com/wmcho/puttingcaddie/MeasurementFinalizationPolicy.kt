...
object MeasurementFinalizationPolicy {
    ...
    fun finalMeasurementFromUi(ui: V31StateMachine.UiModel): FinalMeasurementResult =
        if (ProductFlags.DISTANCE_ONLY_PRODUCT) {
            FinalMeasurementResult(
                ...
                forwardSlope = slopeDecisionDisabled("DISTANCE_ONLY_PRODUCT"),
                lateralSlope = slopeDecisionDisabled("DISTANCE_ONLY_PRODUCT")
            )
        } else {
            FinalMeasurementResult(
                distance = distanceDecisionFromUi(ui),
    ...
    private fun slopeDecisionDisabled(reason: String): SlopeDecision =
        SlopeDecision(
            status = MetricStatus.UNAVAILABLE,
            source = null,
            forwardPct = null,
    ...
    fun logFinalSlopeAxes(final: FinalMeasurementResult) {
        if (ProductFlags.DISTANCE_ONLY_PRODUCT) return
    ...
    fun distanceDecisionFromUi(ui: V31StateMachine.UiModel): DistanceDecision {
        if (ProductFlags.DISTANCE_ONLY_PRODUCT) {
            val candidates =
                listOfNotNull(
                    ui.distanceMeters.takeIf { it.isFinite() && it > 0f },
                    ui.finalDistanceLivePlaneMeters?.takeIf { it.isFinite() && it > 0f },
                    ui.liveRawMeters?.takeIf { it.isFinite() && it > 0f },
                    ui.anchorDistanceMeters?.takeIf { it.isFinite() && it > 0f },
                    ui.multiRayEstimatedDistanceMeters?.takeIf { it.isFinite() && it > 0f }
                )
            val v = candidates.firstOrNull()
            return if (v != null) {
                DistanceDecision(status = MetricStatus.VALID, valueMeters = v, reason = null)
            } else {
                // 제품: 인공 최소값(예: 0.01m) 금지 — SSOT는 없음, 화면 거리는 엔진이 직전 안정값 유지
                DistanceDecision(
                    status = MetricStatus.UNAVAILABLE,
                    valueMeters = null,
                    reason = "distance_only_no_positive_candidate"
                )
            }
        }
