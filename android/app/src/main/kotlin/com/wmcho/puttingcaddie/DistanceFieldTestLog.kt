...
object DistanceFieldTestLog {
    ...
    fun feedbackSnapshot(ui: V31StateMachine.UiModel, extras: ActivityExtras): DistanceFeedbackSnapshot {
        ...
        val (hardGuardPass, hardGuardReason) =
            if (isResult && rawDistanceOk) {
                MeasurementFinalizationPolicy.distanceHardGuardsPass(ui)
            } else {
                true to null
            }
        // stable_403: 엔진 거리가 있으면 OK. 하드 가드는 로그 참고용만.
        val distanceOk = rawDistanceOk
        val distanceMetricStatus =
            if (isResult) {
                distPolicy?.status?.name ?: if (rawDistanceOk) "VALID" else "UNAVAILABLE"
            } else {
                if (rawDistanceOk) "VALID" else "UNAVAILABLE"
            }
        val distanceHardGuardReason = if (!hardGuardPass) hardGuardReason else null
