...
class MeasurementFinalizationPolicyTest {
    ...
    /** 스토어 1.1.5 / DISTANCE_ONLY_PRODUCT: 거리만 VALID, 경사 축은 비활성 */
    @Test
    fun finalMeasurementFromUi_distanceOnlyProduct_disablesSlopeAxes() {
        MeasurementFinalizationPolicy.beginSession(MeasurementFinalizationPolicy.TargetMode.CUP_STANDARD)
        val ui =
            baseUiModel(
                distanceMeters = 3f,
        ...
        assertEquals(MeasurementFinalizationPolicy.MetricStatus.UNAVAILABLE, fm.forwardSlope.status)
        assertEquals("DISTANCE_ONLY_PRODUCT", fm.forwardSlope.reason)
        assertEquals(MeasurementFinalizationPolicy.MetricStatus.UNAVAILABLE, fm.lateralSlope.status)
        assertEquals("DISTANCE_ONLY_PRODUCT", fm.lateralSlope.reason)
    }

    @Test
    fun distanceDecisionFromUi_valid_evenWhenHardGuardWouldFail_stable403Product() {
        MeasurementFinalizationPolicy.beginSession(MeasurementFinalizationPolicy.TargetMode.CUP_STANDARD)
        val ui =
            baseUiModel(
                distanceMeters = 3f,
        ...
        val (pass, reason) = MeasurementFinalizationPolicy.distanceHardGuardsPass(ui)
        assertEquals(false, pass)
        assertEquals("projected_cup_px_below_guard", reason)
    }

    /** 2026-06-12 필드 로그: gateDelta=0.73m, cupPx=54 — 예전엔 재측정 UI, stable은 숫자 표시 */
    @Test
    fun distanceDecisionFromUi_valid_whenFieldLogCupXzExceeds() {
        MeasurementFinalizationPolicy.beginSession(MeasurementFinalizationPolicy.TargetMode.CUP_STANDARD)
        val ui =
            baseUiModel(
                distanceMeters = 3.664506f,
                trackingState = "TRACKING",
                multiRayProjectedCupPx = 54.07f,
                cupCandidateVsLiveHitXZDeltaMAtCommit = 0.728220f,
                slopeExperimentalResult =
                    spreadOnlySlopeResult(sampleSpreadCupM = 0.706503f)
            )
        val d = MeasurementFinalizationPolicy.distanceDecisionFromUi(ui)
        assertEquals(MeasurementFinalizationPolicy.MetricStatus.VALID, d.status)
        assertEquals(3.664506f, d.valueMeters!!, 1e-4f)
        val (pass, reason) = MeasurementFinalizationPolicy.distanceHardGuardsPass(ui)
        assertEquals(false, pass)
        assertEquals("cup_candidate_vs_live_xz_exceeds", reason)
    }

    /** 2026-06-12 필드 로그: spread=0.627m — 예전엔 재측정 UI */
    @Test
    fun distanceDecisionFromUi_valid_whenFieldLogSpreadExceeds() {
        MeasurementFinalizationPolicy.beginSession(MeasurementFinalizationPolicy.TargetMode.CUP_STANDARD)
        val ui =
            baseUiModel(
                distanceMeters = 2.418535f,
                trackingState = "TRACKING",
                multiRayProjectedCupPx = 72.48f,
                cupCandidateVsLiveHitXZDeltaMAtCommit = 0.368875f,
                slopeExperimentalResult =
                    spreadOnlySlopeResult(sampleSpreadCupM = 0.626983f)
            )
        val d = MeasurementFinalizationPolicy.distanceDecisionFromUi(ui)
        assertEquals(MeasurementFinalizationPolicy.MetricStatus.VALID, d.status)
        assertEquals(2.418535f, d.valueMeters!!, 1e-4f)
        val (pass, reason) = MeasurementFinalizationPolicy.distanceHardGuardsPass(ui)
        assertEquals(false, pass)
        assertEquals("sample_spread_cup_exceeds_guard", reason)
    }
    ...
    private fun spreadOnlySlopeResult(sampleSpreadCupM: Float): SlopeInputResult =
        SlopeInputResult(
            ballNormal = null,
            cupNormal = null,
            refNormal = null,
            ...
            lateralPct = null,
            quality = "rejected",
            rejectReason = "plane_drift_too_large",
            sourceId = "LOCAL_SURFACE_FIT",
            experimentalDiagnostics =
                ExperimentalSlopeDiagnostics(sampleSpreadCupM = sampleSpreadCupM)
        )
